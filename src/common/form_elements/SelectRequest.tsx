...
const OptionsWrapper = styled.div<{isShow: boolean}>`
...
const SelectRequest = ({field, form: {setFieldValue}, label, background='#FFFFFF', width, options, ...props}: FieldProps & ISelectProps) => {
    ...
    return (
        <div>
            ...
            <CustomSelect
                ...
                {...props}
                isShow={isShow}
                onClick={() => toggleOptions(!isShow)}
            >
                <span>{field.value.name || 'Choose'}</span><SelectCaretIcon/>

                <OptionsWrapper isShow={isShow}>
                    {
                        options.map(item => (
                                <Option
...
export default React.memo(SelectRequest, (prevProps, nextProps) => {
