...
const RightMessage: React.FC<any> = ({avatar,text,date,setDeleteMessage, id, file, read, suggestion}:{avatar: string,read?:boolean, text:string,date:string,setDeleteMessage:(id:number) => void, id:number, file:string, suggestion?:IOffer}) => {
    ...
    return (
        <div style={{maxHeight:"1000px"}}>
            <Container>
                <div className={"flex-grow-1 d-flex flex-column align-items-end"} style={{marginRight:"15px"}}>
                    ...
                    <div className={"d-flex align-items-center"} style={{marginTop:"9px"}}>
                        <Time>{hours + ":" + minutes}</Time>
                        {read !== undefined ? read ? <ReadIcon /> : <NotReadIcon /> : <></>}
                    </div>
                </div>
