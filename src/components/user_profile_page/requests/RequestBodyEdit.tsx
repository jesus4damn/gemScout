...
import { dataURLtoFile } from "../../../common/functions/dataURLtoFile";
...
const RequestBodyEdit: React.FC<any> = ({request, gems, shapes, setEdit, deleteRequest, changeRequest}:{request: IRequest, gems: IGemstone[], shapes: IShape[], setEdit: () => void, deleteRequest:(id:number) => void, changeRequest:(id:number, request:IRequest) => void}) => {
    const [shapesState, setShapesState] = useState<{id: number, name:string, bool:boolean}[]>(shapes.map((item: IShape) => {
        return {id:item.id,name:item.name,bool:request.shapes.findIndex((itemFind: any) => item.id === itemFind.id) !== -1 ? true : false}
    }));
    ...
    useEffect(() => {
        for(let i = 0; i < request.photos.length; i++){
            if(!request.photos[i].photo.startsWith('data:')){
                toDataUrl(API.baseUrl + request.photos[i].photo, (base64: string) => {
                    request.photos[i].photo = base64;
                })
            }
        }
    ...
    return (
        <Container>
            <Formik
                initialValues={{
                    gem:request.gemstone ? gems.find((item) => item.id === request.gemstone.id)! : gems[0],
                    color:request.color || "",
                    shapes:shapesState,
                    clarity:request.clarity || "",
                    weightFrom:request.weight_from,
                    weightTo:request.weight_to,
                    dimensionFrom:request.dimension_from,
                    dimensionTo:request.dimension_to,
                    treatment:request.treatment || "",
                    quantity:request.quantity,
                    offer_duration:dateFromISO(request.active_until),
                    priceFrom:request.price_from,
                    priceTo:request.price_to,
                    gem_location:request.city,
                    origin:request.origin || "",
                    hideProfile:request.visible || false,
                    hideRequest:false,
                    description:request.buyer_description,
                ...
                onSubmit={async (values : any) => {
                    ...
                    myForm.append('weight_from', values.weightFrom);
                    myForm.append('weight_to', values.weightTo);
                    myForm.append('dimension_from', values.dimensionFrom);
                    myForm.append('dimension_to', values.dimensionTo);
                    myForm.append('price_from', values.priceFrom);
                    myForm.append('price_to', values.priceTo);
                    myForm.append('quantity', values.quantity);
                    myForm.append('city', values.gem_location);
                    if(values.hideProfile){
                        myForm.append('visible', "1");
                    } else {
                        myForm.append('visible', "0");
                    }
                    myForm.append('buyer_description', values.description);
                    let shapes = '';
                    ...
                    myForm.append('gemstone', values.gem.id);
                    myForm.append('color', values.color);
                    myForm.append('origin', values.origin);
                    myForm.append('clarity', values.clarity);
                    myForm.append('treatment', values.treatment);

                    try{
                        const result = await API.put(`/admin/application/${request.id}/change/`, myForm, true);
                        console.log(result.data);
                        changeRequest(request.id, result.data);
                        setEdit();
                    } catch(e){
                        console.log(e);
                    }
                ...
                { ({dirty, values, setFieldValue, submitForm}) =>
                    <Form encType="multipart/form-data">
                        <div style={{paddingRight:"111px"}}>
                            <Inner>
                                <div>
                                    <SelectRequestField name={"Gem"} field={"gem"} options={gems}/>
                                    <div style={{marginBottom:"15px"}}>
                                        <Field
                                            name={`shapes`}
                                            label={"Shapes"}
                                ...
                                <div>
                                    <div style={{display:"flex", justifyContent:"space-between"}}>
                                        <InputField name={"Weight from"} width={"125px"} field={"weightFrom"} right={"ct"} />
                                        <InputField name={"Weight to"} width={"125px"} field={"weightTo"} right={"ct"} />
                                    </div>
                                    <div style={{display:"flex", justifyContent:"space-between"}}>
                                        <InputField name={"Dimension from"} width={"125px"} field={"dimensionFrom"} right={"mm"} />
                                        <InputField name={"Dimension to"} width={"125px"} field={"dimensionTo"} right={"mm"} />
                                    </div>
                                    <div style={{display:"flex", justifyContent:"space-between"}}>
                                        <InputField name={"Price from"} width={"125px"} field={"priceFrom"} right={"$"} />
                                        <InputField name={"Price to"} width={"125px"} field={"priceTo"} right={"$"} />
                                    </div>
                            ...
                            <div className="d-flex flex-wrap w-100">
                                ...
                                <ButtonPlus htmlFor={`${request.id}request_photos`}>
                                    <PlusIcon />
                                </ButtonPlus>
                                <CustomFileInput 
                                    id={`${request.id}request_photos`}
                                    type="file"
                            ...
                            <div className="d-flex justify-content-between w-100">
                                ...
                                {/* <Switch
                                    width={"275px"}
                                    text={"Hide request"}
                                    value={values.hideRequest}
                                    onChange={() => {
                                        setFieldValue('hideRequest',!values.hideRequest);
                                    }} /> */}
                            </div>
                        </div>
                        <div className="d-flex justify-content-between w-100" style={{marginTop:"40px"}}>
                            <div className="text-left">
                                <BaseRedButton onClick={() => deleteRequest(request.id)} type="button" bRadius="21px" padding="8px 39px">Delete</BaseRedButton>
                            </div>
...
export default RequestBodyEdit;